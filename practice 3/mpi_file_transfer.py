...
def sender(comm: MPI.Comm, filepath: str) -> None:
    """
    Rank 0: read a file from local disk and send it to rank 1 using MPI.
    """
    ...
    # First send small metadata as a Python dict
    meta = {"name": filename, "size": file_size}
    comm.send(meta, dest=1, tag=0)

    # Then send the raw bytes as a second message
...
def receiver(comm: MPI.Comm, output_dir: str = ".") -> None:
    """
    Rank 1: receive a file sent from rank 0 and store it on local disk.
    """
